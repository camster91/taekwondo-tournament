...
export type SeedingStrategy = keyof typeof SEEDING_STRATEGIES;
...
export function getKFactor(matchesPlayed: number): number {
  if (matchesPlayed < 10) return RATING_CONFIG.kFactorNew;
  if (matchesPlayed < 30) return RATING_CONFIG.kFactorIntermediate;
  return RATING_CONFIG.kFactorExperienced;
}

// Get age group string for thresholds
export function getAgeGroupKey(ageMin: number, ageMax: number): string {
  return `${ageMin}-${ageMax}`;
}

// Get initial skill estimate for a competitor
export function getInitialSkillEstimate(
  belt: string,
  danRank: number | null,
  ...
  if (belt.toLowerCase() === 'black' && danRank) {
    ...
    const danKey = Math.min(danRank, 6) as keyof typeof blackRatings;
  ...
  const beltRatings: Record<string, number> = {
