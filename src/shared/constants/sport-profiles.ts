export interface SportEventType {
  ...
  name: string;
  description: string;
  hasWeightClasses: boolean;
  isCombat: boolean;
}

export interface SportBeltLevel {
  name: string;
  abbreviation: string;
  rank: number;          // 1 = lowest
  color: string;         // CSS color for UI
  isTopLevel: boolean;   // true = Black Belt equivalent
}

export interface SportBeltConfig {
  levels: SportBeltLevel[];
  hasStripes: boolean;
  hasDanRank: boolean;
  topLevelName: string;  // "Black Belt", "Black Belt (Dan)", "Brown Belt"
  topLevelAbbreviation: string; // "BB", "BB", "BB"
}

export interface SportScoringConfig {
  type: 'points' | 'decision' | 'time' | 'submission';
  hasPenalties: boolean;
  penaltyName: string;   // "Gamjeon", "Shido", "Penalty"
  hasRounds: boolean;
...
export interface SportProfile {
  ...
  beltConfig: SportBeltConfig;
  scoringConfig: SportScoringConfig;
  defaultAgeGroups?: Array<{ min: number; max: number; label: string }>;
}

// ─── TAEKWONDO ────────────────────────────────────────────────────────────────
export const TAEKWONDO_PROFILE: SportProfile = {
  id: 'taekwondo',
  name: 'Taekwondo',
  slug: 'taekwondo',
  icon: '🥋',
...
// ─── KARATE ───────────────────────────────────────────────────────────────────
export const KARATE_PROFILE: SportProfile = {
  id: 'karate',
  name: 'Karate',
  slug: 'karate',
  icon: '🥊',
...
// ─── JUDO ─────────────────────────────────────────────────────────────────────
export const JUDO_PROFILE: SportProfile = {
  id: 'judo',
  name: 'Judo',
  slug: 'judo',
  icon: '🤼',
...
// ─── WRESTLING ────────────────────────────────────────────────────────────────
export const WRESTLING_PROFILE: SportProfile = {
  id: 'wrestling',
  name: 'Wrestling',
  slug: 'wrestling',
  icon: '🤸',
...
// ─── BJJ ──────────────────────────────────────────────────────────────────────
export const BJJ_PROFILE: SportProfile = {
  id: 'bjj',
...
export function getSportEventType(sportSlug: string, eventTypeId: string): SportEventType | undefined {
...
export function getBeltLevelLabel(sportSlug: string, belt: string): string {
  const profile = getSportProfile(sportSlug);
  if (!profile) return belt;
  const level = profile.beltConfig.levels.find(l => l.name.toLowerCase() === belt.toLowerCase());
  return level?.name ?? belt;
}

export function isTopLevelBelt(sportSlug: string, belt: string): boolean {
  const profile = getSportProfile(sportSlug);
  if (!profile) return belt.toLowerCase() === 'black';
