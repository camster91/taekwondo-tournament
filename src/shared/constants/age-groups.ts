// Default age group configurations
...
export const BB_AGE_GROUPS: AgeGroup[] = [
...
export function getAgeGroup(age: number, groups: AgeGroup[] = DEFAULT_AGE_GROUPS): AgeGroup | null {
  return groups.find(g => age >= g.min && age <= g.max) || null;
}

export function calculateAge(dateOfBirth: Date, tournamentDate: Date): number {
