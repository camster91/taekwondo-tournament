...
export interface FairnessFactors {
  weightProximity: number;   // 0-100: How close weights are
  heightProximity: number;   // 0-100: Height differential score
  reachProximity: number;    // 0-100: Reach differential score
  experienceMatch: number;   // 0-100: Experience level similarity
  skillMatch: number;        // 0-100: Skill rating similarity
  schoolDiversity: number;   // 0-100: Different schools = 100
  noRepeatMatchup: number;   // 0-100: No recent rematches = 100
}
...
export interface BracketFairnessReport {
  overallScore: number;
  grade: 'A' | 'B' | 'C' | 'D' | 'F';
  matchScores: Array<{
    round: number;
...
type RegistrationWithCompetitor = Registration & { competitor: Competitor };
 ...
 */
function calculateWeightProximity(
  weight1: number | null,
  weight2: number | null,
  ageGroup: string
): number {
 ...
 */
function calculateHeightProximity(
  height1: number | null,
  height2: number | null,
  ageGroup: string
): number {
 ...
 */
function calculateReachProximity(
  reach1: number | null,
  reach2: number | null
): number {
  ...
  if (diff === 0) return 100;
  if (diff >= PHYSICAL_THRESHOLDS.reach.critical * 2) return 0;
 ...
 */
function calculateExperienceMatch(exp1: number, exp2: number): number {
 ...
 */
function calculateSkillMatch(rating1: number, rating2: number): number {
 ...
 */
function calculateSchoolDiversity(
  school1: string | null,
  school2: string | null
): number {
...
function getGrade(score: number): 'A' | 'B' | 'C' | 'D' | 'F' {
  if (score >= FAIRNESS_THRESHOLDS.excellent) return 'A';
  if (score >= FAIRNESS_THRESHOLDS.good) return 'B';
  if (score >= FAIRNESS_THRESHOLDS.acceptable) return 'C';
  if (score >= FAIRNESS_THRESHOLDS.poor) return 'D';
  return 'F';
}
 ...
 */
export async function calculateMatchFairness(
  prisma: PrismaClient,
  ...
  const rematchCheck = await shouldAvoidRematch(
    prisma,
    comp1.competitorId,
    comp2.competitorId,
    tournamentId
 ...
 */
export async function calculateBracketFairness(
  prisma: PrismaClient,
  divisionId: string,
  bracketStructure: any, // Parsed bracket JSON
  eventType: 'patterns' | 'sparring',
  tournamentId: string,
  ageGroup: string
): Promise<BracketFairnessReport> {
  ...
  for (const a of assignments) {
    const profile = await buildCompetitorProfile(
      prisma,
      a.registration as RegistrationWithCompetitor,
      eventType
    );
    profiles.set(a.registrationId, profile);
  }

  const matchScores: BracketFairnessReport['matchScores'] = [];
  const warnings: string[] = [];
  const recommendations: string[] = [];
  ...
  for (const match of firstRoundMatches) {
    if (!match.competitor1Id || !match.competitor2Id) continue;
    ...
    const fairness = await calculateMatchFairness(
      prisma,
      profile1,
      profile2,
      eventType,
    ...
    if (fairness.grade === 'F') {
      warnings.push(
        `Critical mismatch in match ${match.matchNumber}: ${profile1.lastName} vs ${profile2.lastName}`
      );
    }
  }
 ...
 */
function extractFirstRoundMatches(
  structure: any
): Array<{ matchNumber: number; competitor1Id?: string; competitor2Id?: string }> {
  const matches: Array<{
    matchNumber: number;
  ...
  if (!firstRound) return matches;

  firstRound.forEach((match: any, index: number) => {
    matches.push({
      matchNumber: index + 1,
 ...
 */
export async function quickFairnessCheck(
  prisma: PrismaClient,
  ...
  ageMin: number,
  ageMax: number
): Promise<{ score: number; grade: 'A' | 'B' | 'C' | 'D' | 'F'; warnings: string[] }> {
  ...
  const [profile1, profile2] = await Promise.all([
    buildCompetitorProfile(prisma, reg1 as RegistrationWithCompetitor, eventType),
    buildCompetitorProfile(prisma, reg2 as RegistrationWithCompetitor, eventType),
  ]);

  const ageGroup = getAgeGroupKey(ageMin, ageMax);
  const result = await calculateMatchFairness(
    prisma,
    profile1,
    profile2,
    eventType,
 ...
 */
export async function getDivisionFairnessRecommendations(
  prisma: PrismaClient,
  divisionId: string,
  eventType: 'patterns' | 'sparring',
  tournamentId: string
  ...
  if (poorMatchups.length > 0) {
    recommendations.push(
      `${poorMatchups.length} potential poor matchups detected - consider seeding adjustments`
    );
  }
  ...
  if (eventType === 'sparring') {
    const weights = profiles.filter((p) => p.weight !== null).map((p) => p.weight as number);
    if (weights.length >= 2) {
