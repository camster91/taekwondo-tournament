...
|---|---|---|---|
| Release candidate | Type checks, lint, unit/integration tests, browser workflows, production build, Prisma validation, and clean migrations pass on the release commit | Immutable commit SHA and retained command/check output | Current worktree: 446 unit/integration tests, 27 Chromium workflows, type checks, lint, build, schema validation, and all 7 migrations passed on PostgreSQL 16; commit/push pending. Remote CI unavailable while repository Actions are disabled |
...
| Billing | Managed pilot has an accepted quote/invoice/refund process; self-service launch additionally requires checkout, webhooks, entitlements, taxes, dunning, cancellation, and billing portal | Transaction sandbox tests and server-side entitlement tests | Managed invoicing open; self-service billing absent |
