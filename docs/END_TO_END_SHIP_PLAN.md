...
- ✅ PR #271 (DRAFT): Brand token leftover batch — **Auth/settings cluster migrated**: OrganizationSettings, Profile, UserManagement (gray-*/slate-* → surface-*, semantic color tokens). **Public-facing cluster migrated**: PublicRegister, PublicScoreboard, PublicScoreboardBySlug (automated batch migration: gray/slate → surface, emerald/green → success, red → danger, amber/yellow → warning, blue → info). Dark mode preserved. **Remaining leftovers**: 32 pages with legacy colors (AcceptInvite, AnnouncerView, BracketEditor, CheckIn, Competitors, Dashboard, DirectorDashboard, Divisions, EventPortal, Legal, Login, Results, Schedule, Scorekeeper, SupportTickets, TournamentDetail, TournamentSettings, Waitlist, etc.). Est. 1–2 days for complete migration. Auth/settings + public clusters now fully on brand tokens.
