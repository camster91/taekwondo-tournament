...
- ✅ PR #274: Tournament capacity & waitlist (#191) — **Backend complete**: `Tournament.maxCapacity` + `Tournament.waitlistEnabled` fields; `checkWaitlistStatus()` service; public registration auto-waitlists when full; `POST /api/tournaments/:id/registrations/:regId/promote` with capacity validation, transaction safety, audit logging; `GET /api/tournaments/:id/capacity` status endpoint; migration `20260910_add_capacity_waitlist`. **Frontend complete**: TournamentSettings capacity controls + save state; Waitlist.tsx promotion with ConfirmDialog; PublicRegister capacity messaging (spots left/waitlist/closed). **Tests**: 10 waitlist unit tests + concurrent promotion overflow guard. **VPS deploy note**: Run `prisma migrate deploy` to apply capacity schema migration.
- ✅ PR #278: #128 a11y Slice 1 (axe WCAG 2.2 AA baseline) — **Axe audits complete**: Login, Public Registration, Scorekeeper, Public Scoreboard all pass axe WCAG 2.2 AA automated checks (zero violations). **Baseline coverage**: mobile-chrome viewport, reduced-motion preference. **docs/a11y-release-gates.md created**: Documents Slice 1 gates and VPS verification workflow (GitHub Actions is NOT a hard ship gate). **Fixes shipped**: Label associations, ARIA landmarks, error announcements, keyboard nav basics, autocomplete hints, form semantics.
- ✅ PR #279: API contracts Phase 2 client migration + ship-plan sync (#130) — **Phase 2 COMPLETE**: DirectorDashboard, Scorekeeper, Divisions, BracketEditor migrated to `@/shared/contracts` for type-safe API communication. **api-types.ts deprecated retained**: Old types kept for gradual migration of remaining surfaces. **Phase 3 DEFERRED**: Runtime Zod `.parse()` validation on client and server deferred pending performance benchmarking (DO NOT implement Phase 3 until benchmarked). **docs/contracts.md synced**: Phase 2 marked complete with PR #279.
...
**Overall estimated completion:** ~95% (custom domains API+UI complete #256/#257, live schedule delay propagation #125/#259 complete; day-of operations ~97%; UX Wave 0 trust items #135/#140/#141/#134/#136/#146 complete; day-of trust batch #138 complete, #139 complete; a11y Slice 1 #128 axe baseline shipped #278; contracts Phase 2 #130 complete #279)  
...
**Next Agent Track:** #128 a11y Slice 2 (keyboard-only journeys + 320px/200% zoom + expanded axe coverage) — **Slice 1 COMPLETE** (PR #278): Login, Public Registration, Scorekeeper, Public Scoreboard pass axe WCAG 2.2 AA + baseline keyboard/mobile/reduced-motion. **Slice 2 scope**: (1) Keyboard-only journey completion tests (Tab/Shift+Tab/Enter/Space/arrows); (2) 320 CSS-pixel + 200% zoom/reflow deterministic checks; (3) Expand axe to Check-in and Tournament management surfaces where fixtures allow; (4) Update docs/a11y-release-gates.md for Slice 2. **Remaining #128 work after Slice 2**: Color contrast audits, screen reader testing, touch target sizing, additional surfaces not yet covered. **Contracts Phase 3 DEFERRED** pending performance benchmarking — DO NOT implement runtime validation middleware.
