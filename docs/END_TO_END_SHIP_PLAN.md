...
- 🚧 PR #TBD: Shared component contracts + critical surface migrations (#147 PARTIAL) — **COMPONENT_CONTRACTS.md** created with behavioral contracts for AsyncState/MutationState utilities, Spinner/EmptyState/OperationStatus/Button/ConfirmDialog/Modal/Input/Card components, ARIA requirements, dark mode support, retry patterns, and migration checklist. **3 critical surfaces migrated**: (1) Scorekeeper: replaced ad-hoc "Loading divisions..." text with Spinner + sr-only ARIA, replaced custom error div with OperationStatus + retry, consolidated 2 hand-rolled empty states into EmptyState component; (2) Divisions: added missing isError/refetch to main query (fixes high-risk silent failure), added OperationStatus error banner + retry, added filtered-empty EmptyState for when groupedDivisions is empty; (3) CheckIn: replaced ad-hoc error div with OperationStatus + retry, replaced generic empty text with EmptyState, split true-empty vs filtered-empty messaging based on active filters. Client typecheck passes, async-state tests pass (28/28). **LEFTOVERS**: DirectorDashboard (missing progress isError, ad-hoc attention/SOS banners), Tournaments (finish #262 by wiring unused async-state imports or dropping them, swap error Card to OperationStatus), async-state helpers imported but not yet called in migrated surfaces (only structural migration complete), no #150 design-token refactor (out of scope).
