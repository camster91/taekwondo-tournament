...
- ✅ PR #266: Contextual, recoverable, keyboard-safe onboarding (#145 COMPLETE) — draft-storage utility for recoverable flows (7-day TTL, auto-cleanup, 14 unit tests), onboarding-flow utility for contextual first-run UX (step detection, URL state, 35 unit tests), improved OnboardingChecklist (contextual action buttons, responsive 320px+, keyboard nav, draft recovery notice, terse copy, ARIA labels), improved Tour (responsive mobile, keyboard hints, ARIA labels), wired recovery into OrganizationSettings + Tournaments (load/save/clear drafts, Resume/Discard dialogs). All 49 unit tests pass. Recovery flow works end-to-end.
- ✅ PR #267: Shared component contracts + critical surface migrations (#147 COMPLETE) — **COMPONENT_CONTRACTS.md** created with behavioral contracts for AsyncState/MutationState utilities, Spinner/EmptyState/OperationStatus/Button/ConfirmDialog/Modal/Input/Card components, ARIA requirements, dark mode support, retry patterns, and migration checklist. **5 critical surfaces migrated**: (1) Scorekeeper: replaced ad-hoc "Loading divisions..." text with Spinner + sr-only ARIA, replaced custom error div with OperationStatus + retry, consolidated 2 hand-rolled empty states into EmptyState component; (2) Divisions: added missing isError/refetch to main query (fixes high-risk silent failure), added OperationStatus error banner + retry, added filtered-empty EmptyState for when groupedDivisions is empty; (3) CheckIn: replaced ad-hoc error div with OperationStatus + retry, replaced generic empty text with EmptyState, split true-empty vs filtered-empty messaging based on active filters; (4) DirectorDashboard: added missing isError/refetch to progress query, replaced ad-hoc attention/SOS loading/error banners with Spinner+OperationStatus+retry, replaced generic empty text with EmptyState for rings and divisions, fixed operational query error banner to use OperationStatus+retry; (5) Tournaments: removed unused async-state imports from #262, replaced ad-hoc error Card with OperationStatus+retry. Client typecheck passes, async-state tests pass (28/28). **#147 acceptance met**: DirectorDashboard + Scorekeeper both use shared contracts except documented venue-display escape hatches (none needed). No #150 design-token refactor (out of scope).
- ✅ PR #268: Shared contract follow-up round 2 + wiring completion (#147 COMPLETE) — **Follow-up from PR #267 review**: added draft/finalized/conflicted SaveState unit tests (15 new cases), implemented beforeunload protection in OrganizationSettings (9 integration tests), fixed CheckIn + DirectorDashboard EmptyState props (icon must be component, not element), restored SelectMode dark hover + rounded-lg radius consistency, added Input dark-mode placeholder contrast fix. **Validation pass**: confirmed all 5 surfaces (Scorekeeper, Divisions, CheckIn, DirectorDashboard, Tournaments) correctly use EmptyState, OperationStatus, ConfirmDialog, Spinner per COMPONENT_CONTRACTS. #147 now COMPLETE (both shared contracts + 5 critical surfaces).
- ✅ PR #269: Design token foundation (#150 PARTIAL) — **DESIGN_TOKENS.md** created with canonical token layer (brand, primary, accent, surface, semantic state colors; radius, shadow, typography, motion scales). **Tailwind config extended** with semantic color utilities (`brand-*`, `primary-*`, `accent-*`, `surface-*`, `success`, `warning`, `danger`, `info`). **9 UI primitives migrated** to use tokens: Button, Card, Input, Select, Textarea, Badge, Modal, EmptyState, OperationStatus. **Leftovers:** Full app migration (~20 page components, 50+ hardcoded color replacements per file, est. 2–3 days). #150 foundation complete; full migration deferred.
- ✅ PR #270: Brand token critical surfaces (#150 COMPLETE) — **5 critical surfaces migrated** to canonical design tokens per DESIGN_TOKENS.md: (1) Login/auth screens: hero gradients (`brand-ink`, `primary-800`, `brand-red`), error/success alerts (`danger`, `success`), surface backgrounds (`surface-*`); (2) App shell/navigation: sidebar (`surface-900/950`), header/user menu (`surface-800`), navigation links (`surface-600/300`); (3) Tournaments list: cards/status/delete buttons use `surface-*`/`success`/`danger`; (4) DirectorDashboard: alerts (`danger`/`warning`/`info`), forms (`surface-*`); (5) Scorekeeper: main UI (`surface-900/800`), competitor cards, penalty indicators (`danger`), score inputs. Dark mode preserved. Brand identity (arena ink #0b1220 + belt red #e11d48) now visible throughout. **#150 acceptance criteria complete**: all five specified surfaces use approved token mappings. Remaining ~15 pages (Results, Schedule, CheckIn, BracketEditor, etc.) intentionally deferred.
- ✅ PR #271: Day-of organizer pages brand token migration — DirectorDashboard, Scorekeeper, CheckIn migrated to design tokens; continued #150 rollout
- ✅ PR #272: Auth/settings + public pages design token migration (#150 leftover batch) — Login, VerifyMagicLink, AcceptInvite, Profile, PublicRegister, OrganizationSettings migrated
- ✅ PR #273: Complete brand token migration for all remaining pages — Results, Schedule, BracketEditor, Divisions, TournamentSettings, Competitors, UserManagement migrated; full app now uses DESIGN_TOKENS.md canonical palette
- ✅ PR #274: Tournament capacity & waitlist (#191) — `Tournament.maxCapacity` + `Tournament.waitlistEnabled` fields; public registration auto-waitlists when full; director promotion from waitlist; capacity status endpoint; migration `20260910_add_capacity_waitlist`; **VPS deploy will run `prisma migrate deploy` automatically**
- ✅ PR #275: Documentation reconciliation (#129) — unified runtime setup, migration, and deployment docs across CLAUDE.md, AGENTS.md, deploy scripts, and READMEs
- ✅ PR #276: Truthful public scoreboard failures & freshness (#33) — state machine for loading/stale/fresh/error/empty states; visible staleness warnings; retry + auto-refresh controls; fail-closed for missing tournament/slug
- ✅ PR #277: Shared API response contracts Phase 1 (#130) — established `src/shared/contracts/` with Zod schemas for match, division, tournament, public scoreboard, day-of operations, results; contract tests anchor Prisma query shapes; migrated PublicScoreboard.tsx to shared types; **Phase 2 (client migration) next**
- ✅ PR #278: WCAG 2.2 AA and cross-device a11y gates (refs #128 Slice 1) — established accessibility foundations: WCAG 2.2 AA as ship gate (docs/DEVICE-ACCESSIBILITY-ACCEPTANCE.md), multi-device/AT test matrix (desktop + mobile, Chrome + Firefox + Safari, NVDA/JAWS/VoiceOver), 2 baseline E2E suites (public-register-a11y.spec.ts + scorekeeper-a11y.spec.ts with 15 tests), automated axe-core integration, documented physical-venue testing + sign-off protocol, waiver path for deployment prior to full AT verification
- ✅ PR #279: API contracts Phase 2 client migration (#130) — migrated DirectorDashboard, Scorekeeper, Results, Divisions, BracketEditor from ad-hoc `src/client/utils/api-types.ts` to canonical `@/shared/contracts`; deprecated api-types.ts (retained for rollback safety); added scorekeeper contract test coverage; **Phase 3 runtime validation deferred**
- ✅ PR #281: A11y Slice 2 (#128) — keyboard journey coverage (4 critical flows: login → dashboard → scorekeeper scoring; public reg → submit; check-in → weigh-in; tournament setup → bracket gen), 320px mobile + 200% zoom responsive validation (viewport-zoom-a11y.spec.ts with 8 tests), expanded axe coverage (checkin-a11y.spec.ts + tournament-management-a11y.spec.ts, total 28 a11y E2E tests across 4 suites); **Slice 3+ (physical venue SR verification + every browser×AT combo) remains Cameron/waiver path per docs/DEVICE-ACCESSIBILITY-ACCEPTANCE.md**
...
|----|----------|------|---------------------|--------|
| **Registration** | | | | |
| P2-1 | #227, #191 | **✅ Waitlist management** | Tournament capacity limits (maxCapacity field) + waitlist when full (waitlistEnabled); auto-waitlist on public registration; promote from waitlist with ConfirmDialog + audit trail; organizer UI in TournamentSettings + Waitlist page — **SHIPPED** in PRs #227, #TBD | M |
