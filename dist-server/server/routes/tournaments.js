...
router.put('/:id', authenticate, validateRequest(tournamentUpdateSchema), async (req, res) => {
    const prisma = req.app.locals.prisma;
...
router.post('/:id/registrations', authenticate, validateRequest(registrationSchema), async (req, res) => {
    const prisma = req.app.locals.prisma;
...
router.post('/:id/registrations/bulk', authenticate, validateRequest(bulkRegistrationSchema), async (req, res) => {
    const prisma = req.app.locals.prisma;
    ...
    const registrations = await Promise.all(competitors.map(async (competitor) => {
        const ageAtTournament = calculateAge(competitor.dateOfBirth, tournament.date);
