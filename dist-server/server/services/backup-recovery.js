 ...
 */
export async function backupDivisionState(prisma, tournamentId) {
 ...
 */
export async function restoreDivisionState(prisma, backup) {
    const errors = [];
    let restored = 0;
 ...
 */
export function getBackup(tournamentId) {
    return backupStore.get(tournamentId);
}
/**
 * Clear backup from store
 */
export function clearBackup(tournamentId) {
    backupStore.delete(tournamentId);
}
/**
 * Check if operation would lose data and return warning
 */
export async function checkDataLoss(prisma, tournamentId, operation) {
    switch (operation) {
        case 'regenerate_divisions': {
            ...
            const withManualOverrides = divisions.filter((d) => d.assignments.length > 0);
 ...
 */
export async function validateTournamentState(prisma, tournamentId, requiredState) {
