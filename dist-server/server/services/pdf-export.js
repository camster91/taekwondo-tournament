 ...
 */
export function generateBracketPDF(options) {
    ...
    doc.text(`Generated: ${new Date().toLocaleDateString()}`, pageWidth - margin, pageHeight - 15, { align: 'right' });
    return doc;
}
function drawWinnersBracket(doc, matches, x, y, width, height, showResults) {
    doc.setFontSize(10);
    doc.setFont('helvetica', 'bold');
    ...
    round2.forEach((match, i) => {
        drawMatchBox(doc, match, x + colSpacing, y + r1Spacing / 2 + i * r2Spacing, matchWidth, matchHeight, showResults);
    });
    // Round 3 - 1 match (winners final)
    if (round3.length > 0) {
        drawMatchBox(doc, round3[0], x + colSpacing * 2, y + height / 2 - matchHeight / 2, matchWidth, matchHeight, showResults);
    }
...
function drawLosersBracket(doc, matches, x, y, width, height, showResults) {
    doc.setFontSize(10);
    doc.setFont('helvetica', 'bold');
    ...
    rounds.forEach((roundMatches, round) => {
        ...
        roundMatches.forEach((match, i) => {
            drawMatchBox(doc, match, x + col * colSpacing, y + (i + 1) * spacing - matchHeight / 2, matchWidth, matchHeight, showResults);
        });
        col++;
    });
}
function drawFinals(doc, matches, x, y, showResults) {
    doc.setFontSize(10);
    doc.setFont('helvetica', 'bold');
...
function drawMatchBox(doc, match, x, y, width, height, showResults) {
    // Match box
    doc.setDrawColor(0);
    doc.setLineWidth(1);
 ...
 */
export function generateResultsPDF(tournament, divisions) {
    const doc = new jsPDF({
    ...
    doc.setFontSize(8);
    doc.setFont('helvetica', 'italic');
    doc.text(`Generated: ${new Date().toLocaleDateString()}`, PAGE_WIDTH - MARGIN, PAGE_HEIGHT - 20, { align: 'right' });
    return doc;
}
/**
 ...
 */
export function generateBatchCertificatesPDF(tournament, winners) {
    if (winners.length === 0) {
        const doc = new jsPDF();
 ...
 */
export function generateSchoolReportPDF(data) {
    const doc = new jsPDF({
        orientation: 'portrait',
        unit: 'pt',
        format: 'letter',
...
export function generateBatchBracketsPDF(tournament, brackets, showResults = false) {
