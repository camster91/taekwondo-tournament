...
tar czf "$TARBALL" \
    -C "$PROJECT" \
    dist dist-server prisma prisma.config.ts server.js package.json package-lock.json
...
# .env is read by prisma.config.ts via dotenv/config at config-load
# time. docker --env-file sets process env at runtime but prisma's
# config loader doesn't see those (verified 2026-07-16). The .env
# is generated by deploy-to-vps.sh from /etc/taekwondo.d/app-env
# and is gated to this image only (never published).
