...
router.post('/', authenticate, validateRequest(orgCreateSchema), async (req: AuthenticatedRequest, res: Response) => {
  if (req.user!.role !== 'admin') {
    return res.status(403).json({ error: 'Admin access required' });
  }
...
router.put('/:id', authenticate, validateRequest(orgUpdateSchema), async (req: AuthenticatedRequest, res: Response) => {
  const prisma: PrismaClient = req.app.locals.prisma;
  const orgId = getParam(req.params.id);
...
router.post('/:id/members', authenticate, validateRequest(addMemberSchema), async (req: AuthenticatedRequest, res: Response) => {
  const prisma: PrismaClient = req.app.locals.prisma;
  const orgId = getParam(req.params.id);
...
// Update member role
router.put('/:id/members/:userId', authenticate, validateRequest(memberRoleSchema), async (req: AuthenticatedRequest, res: Response) => {
  const prisma: PrismaClient = req.app.locals.prisma;
  const orgId = getParam(req.params.id);
...
router.get('/:id/tournaments', authenticate, async (req: AuthenticatedRequest, res: Response) => {
  const prisma: PrismaClient = req.app.locals.prisma;
  const orgId = getParam(req.params.id);
