 ...
 */
export async function advanceWinner(prisma, match) {
    if (!match.winnerId) {
    ...
    const matchData = allMatches.find(m => m.matchNumber === match.matchNumber && m.bracketType === match.bracketType);
    if (!matchData) {
    ...
    if (matchData.nextWinnerMatch) {
        const nextWinnerDb = bracket.matches.find(m => m.matchNumber === matchData.nextWinnerMatch);
        if (nextWinnerDb) {
    ...
    if (matchData.nextLoserMatch && match.bracketType === 'winners') {
        const loserId = match.competitor1Id === match.winnerId
            ...
            : match.competitor1Id;
        if (loserId) {
            const nextLoserDb = bracket.matches.find(m => m.matchNumber === matchData.nextLoserMatch);
            if (nextLoserDb) {
    ...
    if (match.bracketType === 'winners' && matchData.nextLoserMatch) {
        ...
        const loserId = match.competitor1Id === match.winnerId
            ...
            : match.competitor1Id;
        if (loserId) {
            const loserMatch = bracket.matches.find(m => m.matchNumber === matchData.nextLoserMatch);
            if (loserMatch) {
 ...
 */
async function advanceToMatch(prisma, matchId, competitorId, fromMatchNumber) {
 ...
 */
async function updateMatchReadyStatus(prisma, bracketId) {
 ...
 */
export async function handleByeMatches(prisma, bracketId) {
    const bracket = await prisma.bracket.findUnique({
        where: { id: bracketId },
    ...
    const firstRoundMatches = bracket.matches.filter(m => m.roundNumber === 1 && m.bracketType === 'winners');
 ...
 */
export async function getBracketPlacements(prisma, bracketId) {
    const bracket = await prisma.bracket.findUnique({
        where: { id: bracketId },
        include: { matches: true },
    });
    ...
    if (resetMatch?.status === 'completed' && resetMatch.winnerId) {
        ...
        if (secondId)
            placements.push({ place: 2, competitorId: secondId });
    }
    ...
    if (losersFinal?.status === 'completed' && losersFinal.winnerId) {
        ...
        if (thirdId)
            placements.push({ place: 3, competitorId: thirdId });
    }
 ...
 */
export function isBracketComplete(matches) {
