...
import { quickFairnessCheck, getDivisionFairnessRecommendations, calculateBracketFairness, } from '../services/fairness-calculator.js';
import { getRatingProfile, batchGetRatings } from '../services/skill-rating.js';
import { getHeadToHead, getRecentOpponents, getRematchRecommendations, } from '../services/matchup-history.js';
const router = Router();
...
router.get('/division/:divisionId/report', async (req, res) => {
    ...
    const recommendations = await getDivisionFairnessRecommendations(prisma, divisionId, eventType, division.tournamentId);
    res.json({
        divisionId,
...
router.post('/matchup/check', authenticate, validateRequest(matchupCheckSchema), async (req, res) => {
    const prisma = req.app.locals.prisma;
    const { registration1Id, registration2Id, eventType, tournamentId, ageMin, ageMax } = req.body;
    const result = await quickFairnessCheck(prisma, registration1Id, registration2Id, eventType, tournamentId, ageMin, ageMax);
    res.json(result);
});
// GET /api/fairness/bracket/:bracketId/analysis
// Analyzes bracket difficulty distribution
router.get('/bracket/:bracketId/analysis', async (req, res) => {
    const prisma = req.app.locals.prisma;
    ...
    const analysis = await calculateBracketFairness(prisma, division.id, structure, eventType, division.tournamentId, ageGroup);
    res.json({
        bracketId,
        divisionName: division.name,
        ...analysis,
...
router.get('/competitor/:competitorId/rating', async (req, res) => {
    const prisma = req.app.locals.prisma;
    const competitorId = getParam(req.params.competitorId);
    const eventType = (getParam(req.query.eventType) || 'sparring');
...
router.get('/competitor/:competitorId/opponents', async (req, res) => {
    const prisma = req.app.locals.prisma;
    const competitorId = getParam(req.params.competitorId);
...
router.get('/head-to-head/:competitor1Id/:competitor2Id', async (req, res) => {
    const prisma = req.app.locals.prisma;
...
router.get('/division/:divisionId/rematch-warnings', async (req, res) => {
    const prisma = req.app.locals.prisma;
    const divisionId = getParam(req.params.divisionId);
    const division = await prisma.division.findUnique({
        where: { id: divisionId },
    ...
    const recommendations = await getRematchRecommendations(prisma, competitorIds, division.tournamentId);
...
router.get('/division/:divisionId/ratings', async (req, res) => {
    const prisma = req.app.locals.prisma;
    const divisionId = getParam(req.params.divisionId);
    const division = await prisma.division.findUnique({
        where: { id: divisionId },
