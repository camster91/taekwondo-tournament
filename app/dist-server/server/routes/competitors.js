...
router.get('/template/mapping', (_req, res) => {
...
router.get('/meta/schools', async (req, res) => {
    const prisma = req.app.locals.prisma;
...
// NOTE: Must be defined BEFORE /:id route to avoid being matched as an ID
router.get('/meta/belts', async (req, res) => {
    const prisma = req.app.locals.prisma;
...
router.post('/', authenticate, validateRequest(competitorCreateSchema), async (req, res) => {
    const prisma = req.app.locals.prisma;
    const { firstName, lastName, gender, dateOfBirth, belt, beltStripe, danRank, heightInches, weightLbs, schoolDojang, specialNeeds, } = req.body;
...
router.put('/:id', authenticate, validateRequest(competitorUpdateSchema), async (req, res) => {
    const prisma = req.app.locals.prisma;
    const { firstName, lastName, gender, dateOfBirth, belt, beltStripe, danRank, heightInches, weightLbs, schoolDojang, specialNeeds, } = req.body;
